...
export default {
  ...
  data() {
    return {
      ...
      enableTooltip: true,
      zoom: 15,
      center: [-6.664772023905216, 106.59664140450836],
      layerData: [],
      geojson: null,
      fillColor: "#0CF9E0",
      tileProviders: tileProviders,
      marker: latLng(-6.664772023905216, 106.59664140450836),
      timeout: undefined,
    };
  },
  mounted() {
  ...
  methods: {
    async loadSomeGeoJson() {
      ...
      const response = await fetch("/batasrwpuraseda.geojson");
  ...
  async created() {
    this.loading = true;
    const response = await fetch("/desapurasedalanduse.geojson");
    const data = await response.json();
    this.geojson = data;
    this.loading = false;
  },
